...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//[5] Test realloc with decreased sizes and next is free
		    //====================================================================//
		    int oureval=0;
		    cprintf(" test realloc_block with FIRST FIT. test 5 i want to decrease size if allocated block and next block is free . must merge");
		    is_correct = 1;
		     realloc_block_FF(startVAs[202],0);// i will free this block
		     // i will decrease size of startVAs[201] it must decrease to be 16  and the remainder merge to the next free block
		     realloc_block_FF(startVAs[201],8);// the size of allocated block will decrease to be 16 and the next free block will be 40
		     if(get_block_size(startVAs[201])!=16)
		     {
		            cprintf("test_realloc_block_FF #5: WRONG REALLOC! size of the block is not correct. Expected Size = %d Found Size = %d \n", 16 ,get_block_size(startVAs[201]));
		            is_correct = 0;
		     }
		     if (get_block_size((char *)startVAs[202]-12) != 40)
		     {
		            cprintf("test_realloc_block_FF #5: WRONG free block size ! size of the  free block is not correct. Expected Size = %d Found Size = %d \n", 40 ,get_block_size((char *)startVAs[202]-12));
		            is_correct = 0;
		     }

		     if(is_correct == 1)
		     {
		         cprintf("test_realloc_block_FF #5: Passed ");
		         oureval+=25;
		     }else
		     {
		         cprintf("test_realloc_block_FF #5: Failed ");
		     }

		     cprintf(" our testing realloc_block with FIRST FIT completed. Evaluation = %d%\n", oureval);

		     //====================================================================//
		     //[6] Test realloc with increased sizes and next is not free
		     //====================================================================//
		cprintf("Test #6: Increase size with next block being full (should free current block and Allocate First Fit)\n");

		is_correct = 1;

		//free up 8192 bytes
		realloc_block_FF(startVAs[10],0);
		realloc_block_FF(startVAs[11],0);

		//try to increase size of one block to 5120 (doesn't fit current block)
		va = realloc_block_FF(startVAs[5],5*kilo);

		//it should reallocate this block to the newly freed block (8192 bytes) and free startVAs[5] and split the 8192 block

		if(is_free_block(startVAs[5]) == 0)
		{
			cprintf("test_realloc_block_FF #6: Expected Block to be free but it's allocated");
			is_correct = 0;
		}
		struct BlockElement * next_block = (void *) va + get_block_size(va);
		if(get_block_size(next_block) != 3064)
		{
			cprintf("test_realloc_block_FF #6: WRONG REALLOC! size of the block is not correct. Expected Size = %d Found Size = %d \n", 3064 ,get_block_size(next_block));
			is_correct = 0;
		}
		if(is_correct == 1)
		 {
			 cprintf("test_realloc_block_FF #6: Passed ");
			 oureval+=25;
		 }else
		 {
			 cprintf("test_realloc_block_FF #6: Failed ");
		 }

		 cprintf(" our testing realloc_block with FIRST FIT completed. Evaluation = %d%\n", oureval);
	 //====================================================================//
	 //[7] Test realloc with increased sizes and next is free without enough size
	 //====================================================================//

	expectedVA=startVAs[1213];
	// current block is 1210 free 1211,1213,1214,1215
	free_block(startVAs[1211]);
	free_block(startVAs[1213]);
	free_block(startVAs[1214]);
	free_block(startVAs[1215]);
	// 1213 has 7*kilo size and 1211 has 21*kilo
	va=realloc_block_FF(startVAs[1210],16*kilo);
	expectedSize=16*kilo+sizeOfMetaData;
	if (check_block(va, expectedVA, expectedSize, 1) == 0)
	{
		is_correct = 0;
		cprintf("test_realloc_block_FF 	MY_TEST: Failed\n");
	}
	if (*(startVAs[blockIndex]) != blockIndex)
	{
		is_correct = 0;
		cprintf("test_realloc_block_FF	 MY_TEST: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
	}
	if(is_free_block(startVAs[1210])==0)
	{
		is_correct = 0;
		cprintf("test_realloc_block_FF	 MY_TEST: must free the block %d\n", blockIndex);
	}
	if(is_correct)
		oureval+=25;
    ...
    //[8] Test realloc with increase sizes and next is free(internal fragmentation)
    		    //====================================================================//
    		    cprintf(" test realloc_block with FIRST FIT. test 8 i want to increase sizes and next is free(internal fragmentation)");
    		    is_correct = 1;
    		     realloc_block_FF(startVAs[212],0);// i will free this block
    		    uint32 list_size=LIST_SIZE(&freeBlocksList);
    		     // i will increase size of startVAs[201] it must increase to be 44 but the next has less than 16 so merge them instead

    		     realloc_block_FF(startVAs[211],44);
    		     if(get_block_size(startVAs[211])!=56)
    		     {
    		            cprintf("test_realloc_block_FF #8: WRONG REALLOC! size of the block is not correct. Expected Size = %d Found Size = %d \n", 16 ,get_block_size(startVAs[201]));
    		            is_correct = 0;
    		     }
    		     if(LIST_SIZE(&freeBlocksList)!=list_size-1)
    		    	 is_correct=0;
    		     if(is_correct == 1)
    		     {
    		         cprintf("test_realloc_block_FF #8: Passed ");
    		         oureval+=25;
    		     }else
    		     {
    		         cprintf("test_realloc_block_FF #8: Failed ");
    		     }

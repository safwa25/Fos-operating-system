...
{
	//TODO: [PROJECT'24.MS1 - #03] [2] SYSTEM
	if(virtual_address != '\0')
	{
		if(virtual_address < USER_HEAP_START || virtual_address + size>= USER_HEAP_MAX)
		{
...
void sys_GoToSleep(struct __semdata* SemData)
{
	...
	enqueue(&SemData->queue,current_process);
	sched();
...
void sys_GoToReady(struct Env_Queue* queue)
{
	...
	{
		struct Env* process = dequeue(queue);
		assert(process != NULL);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sys_free_user_mem:
		sys_free_user_mem((uint32)a1,(uint32)a2);
		return 0;
		break;
	case SYS_sys_allocate_user_mem:
	...
	case SYS_init_queue:
		    ...
		    break;
	case SYS_GoToSleep:
			sys_GoToSleep((struct __semdata*)a1);
			return 0;
			break;
	case SYS_GoToReady:
			sys_GoToReady((struct Env_Queue*)a1);
			return 0;
			break;

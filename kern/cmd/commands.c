...
struct Command commands[] =
{
		...
		{ "schedPRIRR", "Switch & initialize the scheduler to PRIORITY RR with given #priorities, quantum and starvation threshold",command_init_sched,3},
		//**************************************//
...
int command_init_sched(int number_of_arguments, char **arguments){
	uint8 num_priorities = strtol(arguments[1],NULL, 10);;
	uint8 quantum = strtol(arguments[2],NULL, 10);;
	uint8 thresh = strtol(arguments[3],NULL, 10);;
	sched_init_PRIRR(num_priorities,quantum,thresh);
	return 0;
}

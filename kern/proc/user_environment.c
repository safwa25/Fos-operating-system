...
void env_free(struct Env *e)
{
	...
	// 1 - free working set elements
	struct WorkingSetElement* Ws_elemenet ;
	uint32* pageTable=NULL;
	    LIST_FOREACH(Ws_elemenet,&(e->page_WS_list)){
	    	get_page_table(e->env_page_directory,Ws_elemenet->virtual_address,&pageTable);
		    unmap_frame(e->env_page_directory,ROUNDDOWN(Ws_elemenet->virtual_address,PAGE_SIZE));
		    int flag=0;
		...
		LIST_REMOVE(&(e->page_WS_list),Ws_elemenet);
		kfree(Ws_elemenet);
	}
	    for(uint32 i =USER_HEAP_START;i<USER_HEAP_MAX;i+=PAGE_SIZE){
		    unmap_frame(e->env_page_directory,ROUNDDOWN(i,PAGE_SIZE));
	    }
	    for(uint32 i =USER_HEAP_START;i<USER_HEAP_MAX;i+=PAGE_SIZE*1024){
//	    	uint32* pageTable=NULL;
	    	if(get_page_table(e->env_page_directory,i,&pageTable)==TABLE_IN_MEMORY)
			kfree(pageTable);
		}

	//All shared if any (And semaphores)
	acquire_spinlock(&AllShares.shareslock);
	struct Share *shareObj;
	LIST_FOREACH(shareObj,&AllShares.shares_list){
		if(shareObj->ownerID==e->env_id){
			LIST_REMOVE(&AllShares.shares_list,shareObj);
			kfree((void*)shareObj->framesStorage); // free frames stroage
		    kfree((void*)shareObj); // free share itself
		}
	}
...
//	cprintf("Page Table ");
//	uint32* ptr=e->env_page_directory;
//	for (int i=0;i<1024;i++)
//	{
//		uint32* pageTable=NULL;
//		if(get_page_table(e->env_page_directory,(uint32)ptr,&pageTable)==TABLE_IN_MEMORY)
//		kfree(ptr);
//		ptr=ptr+1;
//		cprintf("%d\n",i);
//	}
//	cprintf("After page Table ");
//	//free Directory

	delete_user_kern_stack(e);
	kfree(e->env_page_directory);
	//free user kernel stack
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	pt_set_page_permissions(ptr_user_page_directory,(uint32)va,0,PERM_PRESENT);
	if(va == NULL){
	panic("Kernel Stack Panic");
	}
	return va;

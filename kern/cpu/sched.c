...
struct Env* fos_scheduler_PRIRR()
{
	...
	struct Env *next_env = NULL;
		...
		for(int i = 0; i < num_of_ready_queues;i++){
			//cprintf("current priority is %d and current queue size is %d \n",i,queue_size(&ProcessQueues.env_ready_queues[i]));
			if(queue_size(&ProcessQueues.env_ready_queues[i]) != 0){
				isEmpty = 0;
				next_env = dequeue(&ProcessQueues.env_ready_queues[i]);
				//cprintf("I have selected another env, with the id %d \n",next_env->env_id);
				break;
			}
		}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
		...
		struct Env* curr_env;
			for(int i = 0;i<num_of_ready_queues;i++){
				int curr_size = queue_size(&ProcessQueues.env_ready_queues[i]);
				for(int j = 0;j<curr_size;j++){
					struct Env* temp = dequeue(&ProcessQueues.env_ready_queues[i]);
					temp->ticks += 1;
					//cprintf("My current tick is %d \n",temp->ticks);
					if(temp->ticks > starvThreshold && temp->priority > 0){
						temp->priority--;
						temp->ticks = 0;
						sched_insert_ready(temp);
					}else{
						enqueue(&ProcessQueues.env_ready_queues[i],temp);
					}
				}
			}

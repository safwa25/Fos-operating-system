...
void _main(void)
{
	...
	do
	{
		...
		sys_lock_cons();
		{
			...
			NumOfElements = 250 ;						//Add & Sub
			if (NumOfRep % 3 == 2) 	NumOfElements = 200 ;	//Mul
			...
			{
				cprintf("Select: ") ;
				if (NumOfRep / 3 == 0) 		{ Chose = 'a'; cprintf("a) Ascending\n");}
		...
		sys_lock_cons();
		{
			...
			{
				cprintf("Select: ") ;
				if (NumOfRep % 3 == 0) 		{ Chose = 'a'; cprintf("a) Add (+) \n");}

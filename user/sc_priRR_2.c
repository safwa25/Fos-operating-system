...
struct progInfo programs[] = {
		...
		{"sc_air", {1, 11, 21}, 1},					//SharedMem, Semaphores
		//"fos_data_on_stack",
...
void _main(void)
{
	...
	char getConsLockCntCmd[100] = "__GetConsLockedCnt__";
	char tmpReleaseConsLockCmd[100] = "__tmpReleaseConsLock__";

	sys_lock_cons();
	{
		while (gettst() != TOTAL_INSTANCES)
		{
			uint32 consLockCnt = 0;
			sys_utilities(getConsLockCntCmd, (uint32) &consLockCnt);
			if (consLockCnt == (TOTAL_INSTANCES - gettst()))
			{
				//release console lock to avoid deadlock
				//sys_unlock_cons();
				cprintf("%~\n**********************************************\n"
						"TEMPORARILY RELEASE CONS LOCK TO AVOID DEADLOCK\n"
						"**********************************************\n");
				sys_utilities(tmpReleaseConsLockCmd, 0);
				env_sleep(RANDU(1000, 10000));
				sys_lock_cons();
			}
			env_sleep(RANDU(10000, 20000));
		}
	}
	sys_unlock_cons();
